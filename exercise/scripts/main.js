  ...
  cards.forEach(function(card) {
    card.addEventListener('click', function() {
      ...
      if (selectedCards.length === 2) {
        ...
        var card2 = selectedCards[1];//Adding the next value in the array instead of same
        ...
        card2.classList.remove('is-selected');// card3 is not valid because only 2 arrays are calculated
        selectedCards = [];
      }

      // If we've matched all the cards, display a message.
      if (matchedCards.length >= cards.length) {//Check the count value if it's GE instead of GR
